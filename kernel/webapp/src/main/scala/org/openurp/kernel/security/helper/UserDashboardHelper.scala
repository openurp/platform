 ...
 */
package org.openurp.kernel.security.helper
...
class UserDashboardHelper {
  ...
  def buildDashboard(user: UrpUserBean): Unit = {
  ...
  private def populateMenus(user: UrpUserBean) {
    val menuProfiles = menuService.getProfiles(user)
    ContextHolder.context.attribute("menuProfiles", menuProfiles)
    var menuProfileId: Integer = null
    Params.get("menuProfileId", classOf[Integer]) match {
      case Some(v) => menuProfileId = v
    }
