...
import org.openurp.kernel.security.service.FuncPermissionManager
...
class PermissionAction extends RestfulAction[FuncPermission] {
   ...
   */
  def edit(): String = {
    val roleId = getIntId("role")
    val role = entityDao.get(classOf[Role], roleId)
    val user = entityDao.get(classOf[User], getUserId()).asInstanceOf[UrpUserBean]
    put("manager", user)
