...
import org.beangle.webmvc.api.action.ActionSupport.ERROR
import org.beangle.webmvc.api.context.Params
import org.beangle.webmvc.api.view.View
import org.beangle.webmvc.entity.action.RestfulAction
import org.openurp.kernel.security.helper.UserDashboardHelper
import org.openurp.kernel.security.model.{ Member, UrpUserBean }
 ...
 */
class UserAction extends RestfulAction[UrpUserBean] {
  ...
  override def info(id: String): String = {
    ...
    if (null != userId) {
      ...
      get("user.name") foreach { name =>
        managed = userManager.get(name).orNull.asInstanceOf[UrpUserBean]
      }
    }
    val me = entityDao.get(classOf[UrpUserBean], getUserId())
    if (null != managed) {
  ...
  protected def saveAndForward(entity: User): View = {
    val user = entity.asInstanceOf[UrpUserBean]
