...
[@b.form action="!save" title=labelInfo theme="list"]
  ...
  [@b.textfield label="同级顺序号" name="indexno" value="${menu.lastindex!}" required="true" maxlength="2" check="match('integer').range(1,100)" /]
  ...
  [@b.select label="menu.entry"  name="menu.entry.id" value="${(menu.entry.id)!}" items=resources?sort_by("name") option="id,description" style="width:400px;"  /]
  [@b.select2 label="使用资源" name1st="Resources" name2nd="resourceId" items1st=alternatives?sort_by("name") items2nd = menu.resources?sort_by("name") option="id,description"/]
