...
object RemoteAppService {
  ...
  def validate(ticket: String, token: AuthenticationToken): SimpleUser = {
    val url = Urp.platformBase + "/kernel/" + AppConfig.name + "/validate.json?token=" + ticket
  ...
  def getAppPermissions(app: String): Set[Int] = {
    val url = Urp.platformBase + "/security/" + AppConfig.name + "/func-resources/permission.json?client=" + app
  ...
  def getRoles(principal: String): Set[Int] = {
    val url = Urp.platformBase + "/security/" + AppConfig.name + "/func-resources/permission.json?user=" + principal
    val resources = new collection.mutable.HashSet[Int]
    resources ++= JSON.parse(IOs.readString(new URL(url).openStream())).asInstanceOf[Iterable[Number]].map(n => n.intValue)
    resources.toSet
  }
  ...
  def getResource(resourceName: String): Option[Resource] = {
    val url = Urp.platformBase + "/security/" + AppConfig.name + "/func-resources/info.json?name=" + resourceName
  ...
  def getRolePermissions(roleId: Int): Set[Int] = {
    val url = Urp.platformBase + "/security/" + AppConfig.name + "/func-resources/permission.json?role.id=" + roleId
    val resources = new collection.mutable.HashSet[Int]
    resources ++= JSON.parse(IOs.readString(new URL(url).openStream())).asInstanceOf[Iterable[Number]].map(n => n.intValue)
    resources.toSet
  }
