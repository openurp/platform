...
import java.util.Set
 ...
 class ProfileHelper {
  ...
  UserDataResolver identifierDataResolver
   ...
   ProfileHelper(EntityDao entityDao, ProfileService profileService) {
    super()
   ...
   * 查看限制资源界面
   */
   void populateInfo(List<? extends Profile> profiles) {
    ...
    for (Profile profile : profiles) {
      ...
      fieldMaps.put(Properties.getProperty(profile, "id").toString(), aoFields)
    }
    ContextHelper.put("profiles", profiles)
    ContextHelper.put("fieldMaps", fieldMaps)
  }

   void fillEditInfo(Profile profile, Long userId, boolean isAdmin) {
  ...
  @SuppressWarnings("unchecked")
   void populateSaveInfo(Profile profile, Long userId, Boolean isAdmin) {
    List<Profile> myProfiles = entityDao.get(User.class, userId).getProfiles()
  ...
  private Set<Field> getIgnoreFields(List<Profile> profiles) {
   ...
   void setIdentifierDataResolver(UserDataResolver identifierDataResolver) {
