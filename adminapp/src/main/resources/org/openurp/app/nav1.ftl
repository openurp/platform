...
<aside class="control-sidebar control-sidebar-dark control-sidebar-open" style="display: block;">
    ...
    <div class="tab-content">
      <div id="control-sidebar-theme-options-tab" class="tab-pane active">
        ...
        <div class="form-group">
          <label class="control-sidebar-subheading"><input type="checkbox" checked="true" onchange="sticky_header(this)">固定头部导航</label>
          <label class="control-sidebar-subheading">
            每页数据量<select onchange="setPageSize(this)">
  ...
  <script type="text/javascript">
    ...
    function setPageSize(ele){
       beangle.createCookie("pageSize",ele.value,100);
    }

  beangle.load(["adminlte","urp","urpnav"],function(adminlte,urp,urpnav){
    urp.config.api='${nav.urp.api}';
    var app = {'name':'${nav.app.name}','base':'${nav.app.base}','url':'${nav.app.base}','navStyle':'beangle-webui-bootstrap'}
    var portal={"name":'platform-portal','url':'${nav.urp.portal}','title':'首页'}
    var params={}
    [#list nav.params as k,v]
    params['${k}']='${v}';
    [/#list]
    [#if nav.profiles??]
    urp.init(${nav.profiles},${nav.cookie!'null'});
    if(urp.config.profiles.length>0 && urp.config.profile){
      var default_p = urp.config.profile
    ...
    jQuery(document).ready(function(){
      urpnav.createGroupNav(app,portal,${nav.menusJson},params);
      [#if nav.profiles??]
      urpnav.createProfileNav();
      [/#if]
      urpnav.setup(params);
    });
  });
